...
def doubleNOT(FOL_Tree):
    ...
    if _symbol_value == "NOT":  # if current is NOT
        ...
        if child_value == "NOT":  # if child is also not, set current to its child
            currentNode.set_node(currentNode.get_child_nodes()[0].get_child_nodes()[0].get_element_type(),
                                 currentNode.get_child_nodes()[0].get_child_nodes()[0].get_element_value())

            if len(currentNode.get_child_nodes()[0].get_child_nodes()[0].get_child_nodes()) == 2:
                currentNode.get_child_nodes()[1] = \
                currentNode.get_child_nodes()[0].get_child_nodes()[0].get_child_nodes()[1]

            currentNode.get_child_nodes()[0] = currentNode.get_child_nodes()[0].get_child_nodes()[0].get_child_nodes()[
                0]
...
class Argument(object):
    ...
    def equals(self, arg):
        """
        Checks if the arg and self are the same arguments
        : param arg:
        :return:
        """
...
def unification(p1, p2, replacements):
    """
    Takes two predicates and tries for unifying them which are the same, i.e. P(x1, y1) and -P(x1, y1)
...
test_cases = [
    ...
    [
        "(FORALL x (EXISTS y (EXISTS z (AND (AND (AND (AND (l x y) (l y z)) (r z)) (IMPLIES (p z) (r z))) (IMPLIES (r z) (p z))))))",
        "(FORALL x (FORALL y (FORALL z (AND (EXISTS x (FORALL y (NOT (AND (p y) (l x y))))) (IMPLIES (AND (l x y) (l y z)) (l x z))))))"],
    ["(FORALL x (EXISTS y (p x y)))", "(EXISTS x (FORALL y (NOT (p x y))))"],
    ["(FORALL x (OR (NOT (p a)) (q a)))", "(FORALL x (p x))", "(OR (NOT (p (f a))) (NOT (q a)))"],
    ["(FORALL x (FORALL z (FORALL u (FORALL w (OR (p x (f x) z) (p u w w))))))",
     "(FORALL x (FORALL y (FORALL z (OR (NOT (p x y z)) (NOT (p z z z))))))"]
     """
]
