...
#    plt.yticks(())
    ...
    g.vs['name'] = [str(i) for i in range(n_samples)]  
    if print_labels:
        g.vs['label'] = [str(i) for i in range(n_samples)]     
    ...
    visual_style["layout"] = [ (X[i,0], X[i,1]) for i in range(n_samples)]
    ...
    if not print_artificial_colors:
        fake = Faker()
        colors = [fake.hex_color() for _ in range(len(unique))]
